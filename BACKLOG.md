...
- [ ] **Add-Shift sheet has no visible Close (X) button** (source:persona/Veteran-Val · verified in
  code: every other sheet has a `.back` X; the Add sheet header `~3000` has only title+date, and an
  outside-tap on the scrim *saves* via `dismiss`). Add a `.back` X to the sheet header mirroring the
  other sheets — discoverable, safe exit. Gate-safe + harness-verifiable. **Strong next pick.**
- [ ] **Mobile FAB opens Add-Shift on `days[0]` (often a past day), not today** (source:persona/
  Veteran-Val · `openSheet(keyOf(days[0]))` at the FAB ~2505). Default to today when today is in the
  viewed period: `days.find(d=>keyOf(d)===todayISO()) || days[0]`. Small, but touches openSheet
  default — verify the period-view + empty-state paths still behave.

### P3 (clarity / nice)
- [ ] **New-grad jargon glosses** (source:persona/New-grad-Nia, corroborated by seed
  `take-home-vs-gross`): "FICA" unexplained in onboarding taxes step (~4150) + Settings (~3249) —
  add "Social Security + Medicare — required, same for everyone"; "Gross pay" in the breakdown
  (~2828) — add a "before taxes & take-outs" subtitle; tie the word "differential" to plain language
  in the onboarding differentials step (~4122). Additive copy only.
