...
### P2
- [ ] **Re-share the invite code from the active board** (source:persona/Swap-savvy-Sam · **directly
  closes the recurring "how do I use the pin someone gives me?" feedback** — pghawkins). After the
  one-time "Board created!" screen the invite code is nowhere; a member must leave/recreate to find
  it. `activeGroup.invite_code` (3749) + `copyCode` (3441) already exist — add an "Invite ·
  {code}" ghost button in the active-board header (~3848, next to "+ Post to the board"). Gate-safe
  but **NOT sandbox-drivable** (swap board needs live auth) — verify via bundle + boot + code review,
  same standard as prior swap ships (#27/#28). **Strong next pick.**
