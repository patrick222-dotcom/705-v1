...
| `docs/siri-shortcut.md` | the Siri Shortcut build guide (#71): the Shortcut as a thin shell, the Session B server contract (200 + `status` envelope for Shortcut clients, `meta`/`form_multi`/`dictation` modes, `app_config`, version handshake), the calendar-aware "Plan shifts" flow, test checklist, publish procedure |
